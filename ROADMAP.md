...
**Active milestone — search-resource frontier:** preregister and compare four extensions mapped to the audit: pawn-capture breadth, all opponent replies, all candidate continuations after a forcing reply, and a third forcing ply. Use all 957 loss-game turns only as development material; report depth-12 selected-move regret, move agreement, expanded states, and wall time under explicit caps. Keep the three-feature formula frozen. Select at most one candidate by a locked strength-per-compute rule, then freeze it before a new opening-suite confirmation. Exit when the project can distinguish whether added breadth/depth improves decisions per unit computation or merely spends more states on the same failure modes.

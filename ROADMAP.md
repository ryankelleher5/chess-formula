...
**Active milestone — UCI playing-strength pilot:** the candidate now runs as a real UCI subprocess without changing its frozen formula or search. Before scored play, `uci-playing-pilot-v1` freezes 20 eight-ply openings, reversed-color pairs, 40 games against each of static Locked-3, static Full-16, and Stockfish 18 at 100 nodes/move, a 160-ply draw cap, no evaluation adjudication, five-second UCI timeout/forfeit handling, and 10,000 opening-pair bootstrap resamples. Protocol exit requires zero candidate illegal moves, crashes, and timeouts; strength has no pass threshold and engine-pool Elo must not be presented as human FIDE Elo.

...
**Active milestone — legal move-policy bridge:** freeze the accepted evaluator and construct a deterministic wrapper that always returns a legal root move. Preregister its root-move enumeration, search/node budget, tie-breaking, Stockfish comparison depth, and top-1/top-k agreement and centipawn-regret metrics before accessing an untouched May corpus. April may be used only for implementation development, not final confirmation. Exit when legal coverage, move quality, and compute cost are measured prospectively; only then consider UCI play or game-strength claims.

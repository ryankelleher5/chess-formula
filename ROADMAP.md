...
**Active milestone — untouched Forcing-3 playing confirmation:** freeze the selected three-ply forcing vocabulary, tactical-first ordering, stand-pat behavior, 256-child per-root cap, unchanged Locked-3 coefficients, UCI fault rules, and explicit state/wall-time reporting before playing any scored game. Use a new color-reversed opening suite with no pilot openings. Compare Forcing-3 directly with the accepted two-ply baseline and constrained Stockfish; do not tune from confirmation games. Exit only after legal reliability, opening-paired score uncertainty, engine-pool Elo difference, and strength-per-compute are reported, whether positive or negative.

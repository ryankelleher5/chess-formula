...
**Active milestone — preregistered March compact-rule test:** on 120 untouched March games, compare pooled-development material-only, Locked-3, Locked-4-passed, bounded two-ply/128-child forcing-search variants, and Full-16. Exclude all January/February position overlaps and use paired source-game bootstrap intervals. The passed-pawn term advances only on confirmed overall MAE improvement; forcing search advances only on confirmed forcing-stratum MAE improvement. The source checksum, algorithms, metrics, and decision rules are frozen in `march-compact-rules-v1` before March data access.

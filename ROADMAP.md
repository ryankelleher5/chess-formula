...
**In-progress milestone — minimum transferable handcrafted subset:** nested January selection is complete. Across 30 leakage-safe outer resamples, the selected procedure used a median of four parameters but recovered only 3.23 cp (25.3%) of full-16's 12.75 cp MAE gain over material, rejecting the 90%-retention hypothesis. The prospectively locked subset is `material + space + tempo`; it will not be changed after examining confirmation data.

**Active step — untouched confirmation domain:** train material-only, the frozen three-feature subset, and full-16 coefficients only on January. Evaluate once on a deterministic, license- and checksum-verified February 2013 Lichess sample, excluding exact positions found in January and using game-clustered intervals. Exit the milestone when the complexity frontier reports January nested performance, selection frequency, and February confirmation performance without leakage.

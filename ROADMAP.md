...
**Active milestone — UCI playing-strength pilot:** expose the frozen Searched Locked-3 move policy through a minimal standards-compliant UCI process without changing its formula, legal-root enumeration, tie-breaking, or search. Before playing any rated test games, freeze the opening suite, paired colors, time or fixed-computation control, opponents and baselines, game count, adjudication, illegal-move/timeout handling, and Elo/win-rate uncertainty method. First exit on protocol correctness and zero illegal moves; scientific exit when a reproducible color-balanced match quantifies game strength and resource use without converting a move-regret result into an unsupported Elo claim.

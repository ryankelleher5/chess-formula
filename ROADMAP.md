...
**Active milestone — search-resource frontier:** preregister and compare four extensions mapped to the audit: pawn-capture breadth, all opponent replies, all candidate continuations after a forcing reply, and a third forcing ply. Use all 957 loss-game turns only as development material; report depth-12 selected-move regret, move agreement, expanded states, and wall time under explicit per-root caps of 128 for the baseline/all-captures pair and 256 for the broader/deeper variants. Keep the three-feature formula frozen. An extension is eligible only if it reduces mean regret by at least 10% and its 31-game bootstrap interval excludes zero; select at most one by lowest mean total states, then freeze it before a new opening-suite confirmation. Exit when the project can distinguish whether added breadth/depth improves decisions per unit computation or merely spends more states on the same failure modes.

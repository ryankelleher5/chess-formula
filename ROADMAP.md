...
**Active milestone — deeper-oracle robustness gate:** freeze Searched Locked-3 exactly and test it on an untouched April human corpus labeled with a deeper Stockfish oracle. Preserve the formula vocabulary, pooled-development training policy, forcing definition, two-ply depth, and 128-child cap. Compare static Locked-3, Searched Locked-3, and Full-16 with game-clustered intervals and explicit computation cost. Exit when the March search gain either survives the deeper target or is identified as a shallow-oracle artifact; move-policy work begins only after this gate because stand-pat evaluation does not always emit a legal root move.

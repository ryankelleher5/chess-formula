...
**Active milestone — game-failure audit and search-resource frontier:** use the 31 Stockfish-100 losses only as development material to locate first irreversible swings and classify missed quiet moves, pawn captures, exchanges, king threats, and horizon failures. Compare a small preregistered set of search extensions under explicit state and wall-time accounting; do not change the frozen three-feature formula during this gate. Lock any candidate before a new opening-suite confirmation. Exit when the project can distinguish whether added search breadth/depth yields reproducible playing strength per unit computation or merely spends more states on the same failure modes.

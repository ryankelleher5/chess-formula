...
**Active milestone — failure-directed compact rule discovery:** use the frozen February predictions only to generate hypotheses. Cluster positions where Locked-3 has at least 300 cp error or sharply disagrees with full-16; quantify phase, forcing status, omitted-feature values, and recurring board structures. Do not refit or select a replacement on February. Exit when one or more explicit compact-rule hypotheses are preregistered for a new March corpus, or when the audit finds no stable, interpretable cluster worth adding.

...
**Active milestone — untouched Forcing-3 playing confirmation:** the candidate is frozen as the unchanged Locked-3 formula with three forcing plies, tactical-first ordering, stand-pat outside check, and 256 children per legal root. Twenty new eight-ply openings have unique final rule states and zero final-state or move-sequence overlap with the pilot; every match reverses colors. Play 40 games against the accepted two-ply baseline and 40 against Stockfish-100n under the existing fault/adjudication rules, reporting state and wall-time costs. Forcing-3 confirms only with zero candidate faults, zero opponent forfeits, and a 95% opening-pair-bootstrap score lower bound above 50% against the baseline; Stockfish is an external anchor without a threshold. Do not tune from confirmation games. Exit after the frozen result is reported, whether positive or negative.

...
**Active milestone — fixed-budget search-efficiency curve:** hold the accepted formula fixed and treat Forcing-3 as the strength anchor. Preregister a compact set of forcing-search depths and per-root budgets spanning roughly 100, 300, 1,000, and 3,000 mean decision states. Measure move regret, cap behavior, wall time, and opening-paired game strength with exact algorithm/version provenance. Use completed confirmation games only for development and freeze a new suite before any claim. Exit with a Pareto curve that shows whether the confirmed third ply earns its 5x computation or whether a smaller budget retains most of the gain.
